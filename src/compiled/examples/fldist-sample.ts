...
export function registerReturn(service: {
    run: (arg0: { external_addresses: string[] }, callParams: CallParams<'arg0'>) => Promise<void>;
}): void;
...
export function registerReturn(...args) {
    ...
    peer.callServiceHandler.use(async (req, resp, next) => {
        ...
        if (req.fnName === 'run') {
            ...
            resp.retCode = ResultCodes.success;
            await service.run(req.args[0], callParams);
            resp.result = {};
        }
...
export async function test(node: string, config?: { ttl?: number }): Promise<void>;
export async function test(peer: FluencePeer, node: string, config?: { ttl?: number }): Promise<void>;
