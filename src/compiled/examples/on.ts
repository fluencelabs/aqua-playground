...
export async function getPeerExternalAddresses(client: FluenceClient, otherNodePeerId: string, ttlMsGeneratedArgument?: number): Promise<string[]> {
...
export async function getDistantAddresses(client: FluenceClient, target: string, viaNode: string, ttlMsGeneratedArgument?: number): Promise<string[]> {
    let request: RequestFlow;
    const promise = new Promise<string[]>((resolve, reject) => {
        request = new RequestFlowBuilder()
            .disableInjections()
