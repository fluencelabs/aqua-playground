...
export function registerOp2(service: { identity: (s: number, callParams: CallParams<'s'>) => Promise<void> }): void;
export function registerOp2(
    serviceId: string,
    service: {
        identity: (s: number, callParams: CallParams<'s'>) => Promise<void>;
    },
): void;
export function registerOp2(
    peer: FluencePeer,
...
export function registerOp2(...args) {
    ...
    peer.callServiceHandler.use(async (req, resp, next) => {
        ...
        if (req.fnName === 'identity') {
            ...
            resp.retCode = ResultCodes.success;
            await service.identity(req.args[0], callParams);
            resp.result = {};
        }
